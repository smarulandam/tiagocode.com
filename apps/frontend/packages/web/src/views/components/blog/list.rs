...
pub fn ListSection(
    ...
    .unwrap_or(false) {
            ...
            if !are_articles_empty {
                div { class: "mt-6 grid grid-cols-1 gap-4 lg:mt-4 xl:grid-cols-2",
...
fn ArticleCard(article: Article) -> Element {
    ...
    rsx! {
        article { class: "h-full rounded-2xl border border-[#d6dee8] bg-white p-4 shadow-[0_14px_26px_-22px_rgba(17,28,42,0.26)] transition duration-200 ease-out hover:-translate-y-[2px] hover:border-accent/30 hover:shadow-[0_20px_34px_-26px_rgba(17,28,42,0.32)] sm:flex sm:items-start",
            ...
            div { class: "mt-4 flex flex-grow flex-col md:mt-0 md:pl-7",
                ...
                p { class: "mt-2 leading-[1.74] text-zeus", "{summary}" }
                a {
                    href: slug,
                    target: "_self",
                    class: "mt-4 inline-flex items-center justify-center self-start rounded-xl border border-border/90 bg-surface-soft/60 px-4 py-2 font-mono text-xs font-semibold uppercase tracking-[0.09em] text-primary transition duration-150 ease-out hover:-translate-y-px hover:border-accent/40 hover:bg-accent/10 hover:text-accent",
                    "Read More"
                }
            }
        }
