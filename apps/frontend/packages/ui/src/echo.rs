...
pub fn Echo() -> Element {
    ...
    rsx! {
        section { class: "mx-auto w-full max-w-5xl rounded-3xl border border-slate-200/70 bg-white/90 p-7 shadow-[0_20px_50px_-30px_rgba(15,23,42,0.5)] backdrop-blur sm:p-8",
            div { class: "flex items-start justify-between gap-5",
                div {
                    h4 { class: "font-display text-xl font-semibold text-slate-900",
            ...
            if !response().is_empty() {
                div { class: "mt-6 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3.5",
