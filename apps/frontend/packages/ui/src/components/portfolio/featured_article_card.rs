    ...
    rsx! {
        article { class: "group rounded-[1.05rem] transition duration-[160ms] ease-out hover:border-teal/20 sm:flex sm:items-start sm:gap-5",
            div { class: "relative overflow-hidden rounded-[0.9rem] bg-smoke flex-shrink-0",
                ImageView {
                    image: thumbnail,
                    class: "w-full sm:w-[300px] lg:w-[320px] transition ease-custom duration-500 group-hover:scale-[1.02] group-hover:saturate-[1.03]".to_string(),
                }
            }
            div { class: "mt-4 flex min-w-0 flex-1 flex-col justify-center sm:mt-0",
                span { class: "text-base font-medium leading-6 text-zeus/60", "{published_at}" }
                ...
                div { class: "mt-3",
                    span {
                        class: "inline-flex items-center rounded-full border border-teal/15 bg-teal/8 px-3.5 py-1.5 font-medium text-base leading-none text-teal",
                        "{category}"
                    }
                }
