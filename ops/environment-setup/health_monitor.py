...
if not WEBSITE_DOMAIN or not DISCORD_HEALTH_CHECK_WEBHOOK_URL or not DISCORD_ALERTS_WEBHOOK_URL:
    print("❌ WEBSITE_DOMAIN, DISCORD_HEALTH_CHECK_WEBHOOK_URL, and DISCORD_ALERTS_WEBHOOK_URL environment variables must be set.")
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    for subdomain in subdomains:
        ...
        if website_status == "ok":
            message = f"✅ **{url}** is UP and healthy."
        elif website_status == "timeout":
            message = f"⚠️ **{url}** timed out after {TIMEOUT_SECONDS}s."
        elif website_status == "connection_error":
            message = f"⚠️ **{url}** is unreachable (connection error)."
